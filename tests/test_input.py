...
def expected_values_for_no_input(): 
    ...
    return (
        "", 
        f"usage: {scriptName} [-h] [-f NUM] [-l NUM] [-t] [-i] [-I] [FILE]\n{scriptName}: error: A file or standard input must be provided. Try -h for help.\n"
    )
...
#TODO: verify this on fedora
@patch("sys.stdin.isatty", mock_stdin_isatty)
def test_no_arg_no_stdin(capsys, expected_values_for_no_input): 
    '''
    args = [util_script]
    actual_stdout, actual_stderr = run_util(args)
    expected = "usage: util.py [-h] [-f NUM] [-l NUM] [-t] [-i] [-I] [FILE]\nutil.py: error: A file or standard input must be provided. Try -h for help.\n"
